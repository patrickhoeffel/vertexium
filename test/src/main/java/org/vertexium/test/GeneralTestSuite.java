...
import static org.vertexium.test.GraphTestBase.graph;
...
public abstract class GeneralTestSuite {
    private static final VertexiumLogger LOGGER = VertexiumLoggerFactory.getLogger(GeneralTestSuite.class);
    ...
    public void testAddEdgeWithNullInOutVertices() {
        try {
            ...
            graph.prepareEdge("e1", outVertexId, inVertexId, "label", VISIBILITY_EMPTY)
                    .save(AUTHORIZATIONS_ALL);
        ...
        try {
            ...
            graph.prepareEdge("e1", outVertex, inVertex, "label", VISIBILITY_EMPTY)
                    .save(AUTHORIZATIONS_ALL);
            fail("should throw an exception");
        } catch (Exception ex) {
            assertNotNull(ex);
    ...
    public void testAddEdgeWithNullLabels() {
        ...
        try {
            String label = null;
            Vertex outVertex = graph.addVertex("v1", VISIBILITY_EMPTY, AUTHORIZATIONS_ALL);
            Vertex inVertex = graph.addVertex("v2", VISIBILITY_EMPTY, AUTHORIZATIONS_ALL);
            graph.prepareEdge("e1", outVertex, inVertex, label, VISIBILITY_EMPTY)
                    .save(AUTHORIZATIONS_ALL);
            fail("should throw an exception");
        } catch (Exception ex) {
    ...
    public void testVertexBoost() throws Exception {
        assumeTrue("Boost not supported", edgeBoostSupported);
